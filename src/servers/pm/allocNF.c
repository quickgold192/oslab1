...
PRIVATE struct hole *last_used = NIL_HOLE;
PRIVATE struct hole *lu_prev = NIL_HOLE;
...
phys_clicks clicks;		/* amount of memory requested */
{
   ...
   do {
      ...
      if (last_used == NIL_HOLE) {      /* pointers are at "head" of list */
         prev_ptr = NIL_HOLE;           
         hp = hole_head;  
      }
      else {                            
         prev_ptr = lu_prev;            /* grab the remembered hole, and the corect */
         hp = last_used;                /* previous one before starting the scan  */
      }
	  
      while (!(wrap == 1 && hp == last_used ) && hp->h_base < swap_base) {    
         ...
         if (hp->h_len >= clicks) {
            ...
            if (hp->h_len == 0) {         /* the current hole is going away so */
               last_used = hp->h_next;      /*  the next one becomes the last used  */
               lu_prev = prev_ptr;
               del_slot(prev_ptr, hp);
            }
            else {                        /* if this hole doesn't go away,  */
               last_used = hp;            /*  it is the last one used  */
               lu_prev = prev_ptr;
            }
         ...
         if (hp == NIL_HOLE) {            /* handle the wrap back to the top  */
            hp = hole_head;
            wrap = 1;
         }
...
register struct hole *hp;	/* ptr to hole to merge with its successors */
{
   ...
   if (hp->h_base + hp->h_len == next_ptr->h_base) {
      ...
      /* next_ptr is about to be restored to the freelist (it was merged with hp), 
       * If last_used == next_ptr, make last_used point to the node absorbing
       * next_ptr.
       */
      if( next_ptr == last_used) {
         last_used = hp;
      }
   ...
   if (hp->h_base + hp->h_len == next_ptr->h_base) {
      hp->h_len += next_ptr->h_len;  /* now get third one's mem */
